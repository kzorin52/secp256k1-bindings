...
public static partial class SecP256k1
{
    ...
    public static bool GetPublicKey(ReadOnlySpan<byte> privateKey, bool compressed, Span<byte> serializedPublicKey)
    {
        ...
        var keyDerivationFailed = secp256k1_ec_pubkey_create(Context, publicKey, privateKey) == 0;

        if (keyDerivationFailed) return false;

        var outputSize = (nuint)serializedPublicKey.Length;
        var flags = compressed ? Secp256K1EcCompressed : Secp256K1EcUncompressed;

        var serializationFailed = secp256k1_ec_pubkey_serialize(Context, serializedPublicKey, ref outputSize, publicKey, flags) == 0;

        if (serializationFailed) return false;
    ...
    {
        var buffer = new byte[compressed ? 33 : 65];

        if (GetPublicKey(privateKey, compressed, buffer)) return buffer;
    ...
    {
        if (compactSignature.Length != 64) throw new ArgumentException($"{nameof(compactSignature)} length should be 64");
    ...
    public static byte[]? SignCompact(ReadOnlySpan<byte> messageHash, ReadOnlySpan<byte> privateKey, out int recoveryId) // Compatable with old API
    {
        ...
        if (SignCompact(messageHash, privateKey, buffer, out recoveryId)) return buffer;
    ...
    public static bool RecoverKeyFromCompact(Span<byte> output, ReadOnlySpan<byte> messageHash, Span<byte> compactSignature, int recoveryId, bool compressed)
    {
        Span<byte> recoverableSignature = stackalloc byte[65];
        Span<byte> publicKey = stackalloc byte[64];
        var expectedLength = compressed ? 33 : 65;
        ...
        if (secp256k1_ecdsa_recover(Context, publicKey, recoverableSignature, messageHash) == 0) return false;

        var flags = compressed ? Secp256K1EcCompressed : Secp256K1EcUncompressed;
        var outputSize = (nuint)output.Length;
    ...
    public static unsafe bool Ecdh(ReadOnlySpan<byte> agreement, ReadOnlySpan<byte> publicKey, ReadOnlySpan<byte> privateKey)
    {
        ...
        secp256k1_ecdh_hash_function hashFunctionPtr = (output, x, y, d) =>
        {
            Span<byte> outputSpan = new(output, outputLength);
            Span<byte> xSpan = new(x, 32);
            if (xSpan.Length < 32) return 0;

            xSpan.CopyTo(outputSpan);
            return 1;
        };

        var gch = GCHandle.Alloc(hashFunctionPtr);
        try
        {
    ...
    public static byte[]? Decompress(ReadOnlySpan<byte> compressed) // Compatable with old API
    {
        ...
        if (Decompress(compressed, buffer)) return buffer;
    ...
    /// </summary>
    /// <param name="publicKeyOutput">
    ///     (Output) pointer to a pubkey object. If 1 is returned, it is set to a parsed version of
    ///     input. If not, its value is undefined.
    /// </param>
    ...
    private static bool PublicKeyParse(Span<byte> publicKeyOutput, ReadOnlySpan<byte> serializedPublicKey)
    {
        ...
        if (inputLen != 33 && inputLen != 65) throw new ArgumentException($"{nameof(serializedPublicKey)} must be 33 or 65 bytes");

        if (publicKeyOutput.Length < 64) throw new ArgumentException($"{nameof(publicKeyOutput)} must be {64} bytes");
    ...
    /// </summary>
    /// <param name="serializedPublicKeyOutput">
    ///     65-byte (if compressed==0) or 33-byte (if compressed==1) output to place the
    ///     serialized key in.
    /// </param>
    /// <param name="publicKey">The secp256k1_pubkey initialized public key.</param>
    /// <param name="flags">
    ///     SECP256K1_EC_COMPRESSED if serialization should be in compressed format, otherwise
    ...
    {
        var compressed = (flags & Secp256K1EcCompressed) == Secp256K1EcCompressed;
        var serializedPubKeyLength = compressed ? 33 : 65;

        if (serializedPublicKeyOutput.Length < serializedPubKeyLength)
        {
            var compressedStr = compressed ? "compressed" : "uncompressed";
            throw new ArgumentException($"{nameof(serializedPublicKeyOutput)} ({compressedStr}) must be {serializedPubKeyLength} bytes");
        }

        var expectedInputLength = flags == Secp256K1EcCompressed ? 33 : 64;

        if (publicKey.Length != expectedInputLength) throw new ArgumentException($"{nameof(publicKey)} must be {expectedInputLength} bytes");

        var newLength = (nuint)serializedPubKeyLength;

        var success = secp256k1_ec_pubkey_serialize(Context, serializedPublicKeyOutput, ref newLength, publicKey, flags) == 1;
    ...
    private static void ToPublicKeyArray(Span<byte> serializedKey, ReadOnlySpan<byte> unmanaged)
    {
        ...
        if (!PublicKeyParse(publicKey, uncompressedPrefixedPublicKey)) throw new CryptographicException("Failed parsing public key");

        // Serialize the public key
        if (!PublicKeySerialize(serializedKey, publicKey)) throw new CryptographicException("Failed serializing public key");
    }
    ...
    private unsafe delegate int secp256k1_ecdh_hash_function(void* output, void* x, void* y, IntPtr data);
...
#pragma warning disable CA1401 // P/Invokes should not be visible
    ...
    [LibraryImport(LibraryName)]
    public static partial int secp256k1_ecdsa_sign_recoverable(IntPtr context, ReadOnlySpan<byte> signature, ReadOnlySpan<byte> messageHash, ReadOnlySpan<byte> privateKey, IntPtr nonceFunction,
        IntPtr nonceData);
